/* ==========================================================
   ...
   Stacked single-column layout · 3-row position cards.
   Same data flow as desktop — GET /api/portfolio/builder +
   POST /api/portfolio/builder/explain.
   localStorage: 'mm-portfolio-pins' (shared with desktop)
   ========================================================== */
...
function _renderShell() {
  return (
    '<section class="headline">' +
      ...
      '<p>เอาหุ้นใน watchlist มาจัดเป็นพอร์ต 5 ตัว · 5 sector · 80/20 ตามแนว ดร.นิเวศน์</p>' +
    '</section>' +
    ...
    '<button class="opus-btn" id="pf-opus-btn" type="button">' +
      ...
      '<p>Claude Opus เชื่อมพอร์ตนี้กับ <strong>เสาหลัก 1 — ปันผล 100M</strong> · ~10 วินาที</p>' +
    '</button>' +
    ...
    '<div class="sec-h"><h3>หุ้นใน Watchlist ที่ไม่เลือก</h3><span class="hint" id="pf-bench-hint"></span></div>' +
...
function _updateHints(root, data) {
  ...
  if (posHint) posHint.textContent = 'anchor ' + anchors + ' · supp ' + supporting + ' · tail ' + tails;
  const benchHint = root.querySelector('#pf-bench-hint');
  if (benchHint) benchHint.textContent = (data.bench || []).length + ' ตัว';
}
...
function _renderSourceBanner(host, source) {
  ...
  const hoursAgo = (source.scan_hours_ago !== null && source.scan_hours_ago !== undefined)
    ? (source.scan_hours_ago + ' ชม. ก่อน')
    : 'ไม่ทราบ';
  host.innerHTML =
    '<div class="src-banner">' +
      '<span class="ic">🗂</span>' +
      '<span class="txt"><strong>' + count + ' หุ้นใน watchlist</strong> · scan ' + esc(hoursAgo) + '</span>' +
...
function _renderPinChips(host, pins) {
  ...
  for (let i = 0; i < pins.length; i++) {
    const p = pins[i];
    chips += '<span class="chip pin">' + esc(p) + ' <span class="x" data-mm-pin-x="' + esc(p) + '" role="button" aria-label="Remove pin">×</span></span>';
  }
  chips += '<span class="chip add" id="pf-pin-add" role="button">+ เพิ่ม pin</span>';
  host.innerHTML =
    '<div class="chip-section">' +
      '<div class="chip-title">ปักหมุด <span class="meta" style="color:var(--fg-dim);font-weight:500;text-transform:none;letter-spacing:0">บังคับใส่พอร์ต</span></div>' +
    ...
    html +=
      '<div class="pos">' +
        '<div class="pos-row1">' +
          ...
          '<div class="pos-weight ' + weightCls + '">' + weightPct + '%</div>' +
        '</div>' +
        '<div class="pos-row2">' +
        ...
        '</div>' +
        '<div class="pos-tags">' + tagsHtml +
          '<span class="pos-score" style="margin-left:auto"><span class="s-dot ' + (p.score_dot || 'b') + '">' + (p.score == null ? '?' : p.score) + '</span>Niwes</span>' +
        '</div>' +
        '<div class="pos-reason">' + esc(p.reason || '') + '</div>' +
...
function _renderBench(host, bench) {
  if (!host) return;
  if (!bench.length) {
    host.innerHTML = '<div style="padding:14px;color:var(--fg-dim);font-style:italic;font-size:0.9rem">หุ้นใน watchlist ทั้งหมดติดพอร์ต</div>';
    return;
  }
  const esc = window.MMUtils.escapeHtml;
  let html = '';
...
function _openPinModal(root) {
  const html =
    '<p style="color:var(--fg-secondary);margin-bottom:var(--sp-4);font-size:0.95rem">กรอก symbol (เช่น "QH") — ระบบเติม .BK ให้</p>' +
  ...
  window.MMComponents.openModal(html, {
    ...
    dek: 'หุ้นที่ปักหมุดจะถูกใส่พอร์ตในช่อง sector ของมันเสมอ.'
  });
...
async function _openOpusModal(root) {
  ...
  try {
    ...
    for (let i = 0; i < paragraphs.length; i++) {
      ...
      if (!para) continue;
      html += '<p style="margin-bottom:var(--sp-3);line-height:1.65;font-size:0.95rem">' + esc(para) + '</p>';
    }
    html += '<div style="margin-top:var(--sp-4);font-size:0.8rem;color:var(--fg-dim)">analyzed_at: ' +
