...
function _updateMasthead(status) {
  ...
  host.innerHTML = window.MMComponents.renderMasthead({ active: 'report' }, window.__MM_ME);
}
...
function _renderAnalyzeLoadingInner() {
  return (
    '<div style="padding:32px 0;text-align:center">' +
      '<div style="display:inline-block;width:28px;height:28px;border:3px solid var(--c-positive-soft);border-top-color:var(--c-positive);border-radius:50%;animation:v6spin 1s linear infinite"></div>' +
      '<style>@keyframes v6spin{to{transform:rotate(360deg)}}</style>' +
      '<div style="font-family:var(--font-mono);font-size:var(--fs-xs);letter-spacing:0.14em;text-transform:uppercase;color:var(--fg-dim);margin-top:12px">กำลังวิเคราะห์ · Claude Opus</div>' +
    '</div>'
  );
}
...
// ---------- Deep Analyze inline UX ----------

var _deepState = { pollHandle: null };
...
function _stopPoll() {
  if (_deepState.pollHandle) {
    clearInterval(_deepState.pollHandle);
    _deepState.pollHandle = null;
  }
}

function _wireDeepAnalyze(sym) {
  var block = document.getElementById('v6-deep-analyze');
  if (!block) return;
  _stopPoll();
...
function _pollAnalysis(sym, block) {
  _stopPoll();
  var elapsed = 0;
  _deepState.pollHandle = setInterval(async function () {
    elapsed += 5;
    try {
