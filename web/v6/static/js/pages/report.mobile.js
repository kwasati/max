...
function _updateMasthead(status) {
  ...
  host.innerHTML = window.MMComponents.renderMobileMastHead(window.__MM_ME, _extractSymbol() || 'Report');
  ...
  return (
    '<section class="report-hero">' +
      ...
      '<div class="report-hero-right">' +
        ...
        '<span class="v6-star" id="v6-mhero-star" data-sym="' + sym + '" data-watched="' + (stock.user_in_watchlist ? 'true' : 'false') + '" title="Toggle watchlist">★</span>' +
      '</div>' +
    '</section>'
  );
}
...
function _wireHeroStar(sym) {
  var el = document.getElementById('v6-mhero-star');
  if (!el) return;
